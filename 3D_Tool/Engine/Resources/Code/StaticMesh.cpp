...
void CStaticMesh::Render_Meshes(LPD3DXEFFECT & pEffect)
{
	for (_ulong i = 0; i < m_dwSubsetCnt; ++i)
	{
		...
		if(Find_Cross(m_pMtrl[i].pTextureFilename))
			iPass = 2;
		if(bConveyor = FInd_Conveyor(m_pMtrl[i].pTextureFilename))
			iPass = 3;
		if (bConveyor = FInd_ConveyorCorner(m_pMtrl[i].pTextureFilename))
			iPass = 4;
		//if (FInd_Glass(m_pMtrl[i].pTextureFilename))
		//	iPass = 5;
...
_bool CStaticMesh::Find_Cross(const char * pFileName)
{
	_uint	iLength = strlen(pFileName);

	string TempFileName = pFileName;

	_int iSearch = TempFileName.find("cross");
...
_bool CStaticMesh::FInd_Conveyor(const char * pFileName)
{
	_uint	iLength = strlen(pFileName);

	string TempFileName = pFileName;

	_int iSearch = TempFileName.find("conveyormatflat");
...
_bool CStaticMesh::FInd_ConveyorCorner(const char * pFileName)
{
	_uint	iLength = strlen(pFileName);

	string TempFileName = pFileName;

	_int iSearch = TempFileName.find("conveyormatflatcorner");
...
_bool CStaticMesh::FInd_Glass(const char * pFileName)
{
	_uint	iLength = strlen(pFileName);

	string TempFileName = pFileName;

	_int iSearch = TempFileName.find("glass");
