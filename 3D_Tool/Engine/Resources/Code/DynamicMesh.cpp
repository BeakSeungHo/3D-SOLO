...
void CDynamicMesh::Render_Meshes(LPD3DXEFFECT & pEffect)
{
	for (auto& iter : m_MeshContainerList)
	{
		...
		//_ulong	dwFVF = pMeshContainer->pOriMesh->GetFVF();	// 메쉬의 정점 정보를 얻어오는 함수

		//m_dwNumVtx = pMeshContainer->pOriMesh->GetNumVertices();	// 현재 메쉬가 지닌 정점의 개수를 반환하는 함수
		...
		//pMeshContainer->pOriMesh->GetDeclaration(Decl); // 메쉬 정점 안에 들어있는 fvf 정보를 얻어오는 함수
		...
		//{
		//	m_pVtxPos[i] = *((_vec3*)(((_ubyte*)pSrcVtx) + (m_dwStride * i + sOffset)));
		//}
