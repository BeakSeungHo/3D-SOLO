...
USING(Engine)
Engine::CColliderBox::CColliderBox(LPDIRECT3DDEVICE9 pGraphicDev)
	: m_pGraphicDev(pGraphicDev)
{
	m_pGraphicDev->AddRef();
}

Engine::CColliderBox::~CColliderBox(void)
{

}

HRESULT Engine::CColliderBox::Ready_Collider(const _vec3* pPos, const _vec3* vMin, const _vec3* vMax)
{
	m_vMin = *pPos + *vMin;
	m_vMax = *pPos + *vMax;
...
void Engine::CColliderBox::Render_Collider(COLLTYPE eType, const _matrix* pColliderBoxMatrix)
{
	m_matWorld = *pColliderBoxMatrix;

#ifdef _DEBUG
	m_pGraphicDev->SetRenderState(D3DRS_LIGHTING, FALSE);
	m_pGraphicDev->SetTransform(D3DTS_WORLD, pColliderBoxMatrix);
...
Engine::CColliderBox* Engine::CColliderBox::Create(LPDIRECT3DDEVICE9 pGraphicDev, const _vec3* pPos, const _vec3* vMin,	const _vec3* vMax)
{
...
void Engine::CColliderBox::Free(void)
{
	#ifdef _DEBUG
	Safe_Release(m_pVB);
	Safe_Release(m_pIB);
