...
CGameObject * CManagement::Get_GameObj(const _tchar * pLayerTag, const _tchar * pObjTag)
{
	if (nullptr == m_pScene)
		return nullptr;

	return m_pScene->Get_GameObj(pLayerTag, pObjTag);
}
...
HRESULT Engine::CManagement::Ready_Management(LPDIRECT3DDEVICE9& pGraphicDev)
{
	...
	FAILED_CHECK_RETURN(Ready_RenderTarget(pGraphicDev, L"Target_Albedo", ViewPort.Width, ViewPort.Height, D3DFMT_A16B16G16R16F, D3DXCOLOR(0.5f, 0.5f, 0.5f, 0.f)), E_FAIL);
	...
	pShader = CShader::Create(pGraphicDev, L"../../Reference/Header/Shader_Conveyor.hpp");
	NULL_CHECK_RETURN(pShader, E_FAIL);
	FAILED_CHECK_RETURN(Engine::Ready_Prototype(L"Shader_Conveyor", pShader), E_FAIL);

	pShader = CShader::Create(pGraphicDev, L"../../Reference/Header/Shader_BlinkTile.hpp");
	NULL_CHECK_RETURN(pShader, E_FAIL);
	FAILED_CHECK_RETURN(Engine::Ready_Prototype(L"Shader_BlinkTile", pShader), E_FAIL);
