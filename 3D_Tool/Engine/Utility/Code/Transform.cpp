...
void CTransform::Lerp_Move_Pos(_vec3*  pEnd)
{
	D3DXVec3Lerp(pEnd, &m_vInfo[INFO_POS], pEnd, 0.1f);

	m_vInfo[INFO_POS] = *pEnd;
}
...
void CTransform::FixingRotation(ROTATION eType, const _float & fAngle)
{
	*(((_float*)&m_vAngle) + eType) = fAngle;
}
...
const _matrix* CTransform::Compute_LookAtTarget(const _vec3 * pTargetPos)
{
	...
	return D3DXMatrixRotationAxis(&matRot,
									D3DXVec3Cross(&vAxis, &m_vInfo[INFO_UP], &vDir),
										...
										D3DXVec3Normalize(&vUp, &m_vInfo[INFO_UP]))) * 10.f);
}
...
void CTransform::Set_Up(const _vec3 * pUp)
{
	m_vInfo[INFO_UP] = *pUp;
}
