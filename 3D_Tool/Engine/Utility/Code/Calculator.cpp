...
_bool CCalculator::Collision_AABB(CColliderBox * pSourBoxCom)
{
	_vec3		vDestMin, vDestMax, vSourMin, vSourMax;
	_float		fMin = 0.f, fMax = 0.f;

	D3DXVec3TransformCoord(&vSourMin, pSourBoxCom->Get_Min(), pSourBoxCom->Get_ColliderBoxMatrix());
	D3DXVec3TransformCoord(&vSourMax, pSourBoxCom->Get_Max(), pSourBoxCom->Get_ColliderBoxMatrix());
...
_bool CCalculator::Coliision_Sphere(const _vec3 * pDestPos, const _float* fDestRadius, const _vec3 * pSourPos, const _float* fSourRadius)
{
	if(D3DXVec3Length(&(*pDestPos - *pSourPos)) > *fDestRadius*0.01f + *fSourRadius *0.01f)
		return false;
